/**
 * Suite node mesh — QNM-BUILD-1.0 Live Nodes contract + PROXY.
 ...
 * Whitestone product Worker. Author: Aziel Eliab only.
 */
...
export const MESH_PRODUCT = "whitestone";
...
export const DEFAULT_RUNTIME_ORIGIN = RUNTIME;
...
export const MESH_PROXY_ROUTES = Object.freeze([
  ...
  { path: MESH_STATUS_PATH, methods: ["get", "head"], op: "status", summary: "PROXY alias of GET /v1/mesh. Not a local op." },
  { path: MESH_NODES_PATH, methods: ["get", "head"], op: "nodes", summary: "PROXY to aziel-runtime GET /v1/mesh/nodes. Live Nodes (5-minute presence). Not a local op." },
...
export function meshPointer() {
  return {
    ...
    origin: RUNTIME + MESH_PATH,
    note: "PROXY to aziel-runtime /v1/mesh/* via AZIEL_RUNTIME. Not a local op. Not AnonBroadcast. Not AZMail's product-local ring. Whitestone remains a standalone ephemeral pro se advisor (not a FragGate engine). Full node process is local qnm-node/. QNS-CD-1.0 photon QNS1 packet transfer is a hub cite / Worker mesh cross-map only — no public qnsd proxy. " + MESH_NOTE,
...
export function meshOpenApiPaths() {
  ...
  for (const route of MESH_PROXY_ROUTES) {
    ...
    for (const method of route.methods) {
      entry[method] = {
        operationId: "whitestone_mesh_" + route.op + (method === "head" ? "_head" : "") + "_proxy",
...
export function runtimeOrigin(env) {
  const raw = env && typeof env.AZIEL_RUNTIME_ORIGIN === "string" ? env.AZIEL_RUNTIME_ORIGIN.trim() : "";
...
export function joinOriginUrl(base, pathAndQuery) {
  const origin = String(base == null ? "" : base).trim().replace(/\/+$/, "") || DEFAULT_RUNTIME_ORIGIN;
...
export async function originFetch(env, pathAndQuery, init, request) {
  ...
  headers.set("X-Aziel-Runtime-Via", "whitestone-download-tracker");
